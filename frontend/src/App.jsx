...
const confirmShare = async () => {
  ...
  try {
    ...
    // 检查是否有正反面的图像数据
    const hasFront = pgFrontRef.current && !isGraphicsEmpty(pgFrontRef.current);
    const hasBack = pgBackRef.current && !isGraphicsEmpty(pgBackRef.current);
    
    console.log('hasFront:', hasFront, 'hasBack:', hasBack); // 调试用
    ...
    if (hasFront && hasBack) {
      ...
      imgFront.src = pgFrontRef.current.canvas.toDataURL();
      await new Promise(resolve => { imgFront.onload = resolve; });
      
      // 加载背面图
      const imgBack = new Image();
      imgBack.src = pgBackRef.current.canvas.toDataURL();
      ...
      // 单面展示
      const activePg = hasFront ? pgFrontRef.current : pgBackRef.current;
      const sideLabel = hasFront ? '正面' : '背面';
      const img = new Image();
      img.src = activePg.canvas.toDataURL();
...
// 辅助函数：检查 graphics 是否为空
const isGraphicsEmpty = (pg) => {
  if (!pg) return true;
  ...
  try {
    ...
    // 检查是否有任何非透明的像素（alpha > 0）
    for (let i = 3; i < data.length; i += 4) {
      if (data[i] > 0) {
        return false; // 找到了非透明像素，说明有绘画内容
      }
    }
    return true; // 全是透明的
  } catch (e) {
    console.error("检测画布内容失败:", e);
    return false; // 出错时保守地认为有内容
  }
};
            ...
            return (
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                  <img 
                    src={pgFrontRef.current.canvas.toDataURL()} 
                ...
                <div style={{ textAlign: 'center' }}>
                  <img 
                    src={pgBackRef.current.canvas.toDataURL()} 
                    style={{ width: '180px', height: 'auto', borderRadius: '8px', border: '1px solid #444' }}
                    alt="背面"
                  />
