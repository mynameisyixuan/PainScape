    ...
    try {
      ...
      const checkPageBreak = (heightNeeded) => {
        if (yPos + heightNeeded > pageHeight - 20) {
          ...
          doc.text("PainScape 痛觉档案 - 由 AI 辅助生成，仅供参考", pageWidth / 2, pageHeight - 10, { align: 'center' });
        }
      };
  ...
  const handleRefine = async (fieldKey) => {
    ...
    try {
      ...
      if (refined) {
        ...
        showToast('AI 返回了空内容，请重试');
      }
    } catch (err) {
      console.error('优化失败:', err);
      // 【关键修复】：给用户明确的失败提示
      showToast('❌ AI 优化失败，可能接口未部署或网络异常');
    } finally {
      setRefiningField(null); // 无论成功失败，都重置加载状态
    }
  };
  ...
  const TrendSummary = ({ history }) => {
    ...
    return (
      <div style={{ background: '#1a1a1a', borderRadius: '12px', padding: '15px', marginBottom: '20px', border: '1px solid #333' }}>
        <p style={{ color: '#fff', fontWeight: 'bold', margin: '0 0 10px 0', fontSize: '13px' }}>📊 近期规律</p>
        <div style={{ display: 'flex', gap: '15px' }}>
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ color: '#ef9a9a', fontSize: '20px', fontWeight: 'bold' }}>{dominant[0]}</div>
            <div style={{ color: '#666', fontSize: '11px', marginTop: '4px' }}>最常见痛感</div>
          </div>
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ color: '#90caf9', fontSize: '20px', fontWeight: 'bold' }}>~{avgGap}天</div>
            <div style={{ color: '#666', fontSize: '11px', marginTop: '4px' }}>平均发作间隔</div>
          </div>
          <div style={{ flex: 1, textAlign: 'center' }}>
            <div style={{ color: '#a5d6a7', fontSize: '20px', fontWeight: 'bold' }}>{history.length}</div>
            <div style={{ color: '#666', fontSize: '11px', marginTop: '4px' }}>总记录次数</div>
          </div>
        </div>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 100, pointerEvents: 'auto' }}>
        ...
        {page === "onboarding" && (
          <div style={{ pointerEvents: 'auto', background: '#0a0a0a', width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }}>
              ...
              {showContent === 'medical' && (
                <div style={{ width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {/* 月经周期选择 */}
                    <div>
                      <span style={{ color: '#888', fontSize: '12px', display: 'block', marginBottom: '8px' }}>📅 今天是月经第几天？（可选）</span>
                    ...
                    <span style={{ color: '#666', fontSize: '11px', opacity: 0.6 }}>温和：安抚为主 / 直接：只说方法</span>
                  </div>
                </div>
              )}
            ...
            <button
              onClick={() => {
                setShowContent('preference'); // 重置 showContent 状态
                setPage("canvas"); // 切换到 drawing 页面
              }}
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 10, pointerEvents: 'auto' }}>
            ...
            <div style={{ pointerEvents: 'auto', position: 'absolute', right: '20px', top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '180px', width: '30px', position: 'relative', justifyContent: 'flex-end' }}>
                <div style={{ color: '#888', fontSize: '9px', marginBottom: '4px', writingMode: 'vertical-rl' }}>负荷</div>
        ...
        {page === "quickLog" && (
          <div style={{ pointerEvents: 'auto', background: '#0a0a0a', width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }}>
            <h2 style={{ color: '#fff', marginBottom: '5px' }}>⚡ 快速记录</h2>
            <p style={{ color: '#666', fontSize: '12px', marginBottom: '30px' }}>不需要绘画，3秒捕捉此刻的真实痛感</p>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '30px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {Object.keys(BRUSHES).filter(k => k !== 'eraser').map(key => (
                ...
                >
                  {BRUSHES[key].icon} {PAIN_NAME_MAP[key]}
                </button>
              ))}
            </div>
        ...
        {page === "history" && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', position: 'sticky', top: 0, background: '#0a0a0a', zIndex: 10, paddingBottom: '10px' }}>
              ...
              <button style={{ margin: 0, padding: '6px 12px', width: 'auto', background: '#2196f3', borderColor: '#2196f3', fontSize: '12px' }} onClick={exportHistoryPDF}>📄 导出PDF</button>
              <button className="retry-btn" style={{ margin: 0, padding: '6px 15px', width: 'auto' }} onClick={() => setPage('onboarding')}>返回</button>
            </div>
            {/* 【新增】：趋势概览 */}
              ...
              <div style={{
                ...
                {history.length > 1 && (
                  <div style={{ marginTop: '20px', borderTop: '1px solid #333', paddingTop: '15px' }}>
                    ...
                    >
                      {showCompare ? '收起对比' : '📊 与上次发作对比'}
                    </button>

                    {showCompare && (() => {
                      // 找到当前记录在历史中的索引
                      const currentIndex = history.findIndex(h => h.id === viewingDiary.id);
                      // 获取上一条记录（历史记录是按时间倒序排列的，所以 currentIndex + 1 是更早的那次）
                      const previousRecord = currentIndex < history.length - 1 ? history[currentIndex + 1] : null;

                      if (!previousRecord) {
                        return <p style={{ color: '#666', fontSize: '12px', textAlign: 'center', marginTop: '10px' }}>没有更早的记录可对比</p>;
                      }
                  ...
                  <div style={{ background: '#1a1a1a', padding: '15px', borderRadius: '12px', marginTop: '12px', border: '1px solid #333' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                      ...
                      <button
                        ...
                        onClick={() => handleSaveExperience(viewingPost)} // 【修改】：传入当前帖子对象
                      >发布经验</button>
                    </div>
                  </div>
                )}
